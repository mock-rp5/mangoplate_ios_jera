...
class MangoPickDataManager {
  ...
  func getStory(_ parameters: StoryRequest, viewController: ThirdRestaurantDetailCell) {
    viewController.showIndicator()
    AF.request("\(Constant.DEV_BASE_URL)/app/mangopicks/story", method: .get, parameters: parameters, headers: Constant.HEADERS)
      .validate()
      .responseDecodable(of: StoryResponse.self) { response in
        switch response.result {
        case .success(let result):
  ...
  func getTopList(_ parameters: TopListRequest,  viewController: ThirdRestaurantDetailCell) {
    viewController.showIndicator()
    AF.request("\(Constant.DEV_BASE_URL)/app/mangopicks/toplist", method: .get, parameters: parameters, headers: Constant.HEADERS)
      .validate()
      .responseDecodable(of: TopListResponse.self) { response in
        switch response.result {
        case .success(let result):
